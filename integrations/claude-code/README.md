...
happen — all through your local tts-daemon.
...
1. Start the gateway (`tts-daemon serve`) — or install it as a service, see
...
| ----------------------------- | ------------------------ | ------------------------------ |
| `TTS_DAEMON_URL`             | `http://127.0.0.1:5111`  | Gateway base URL               |
| `TTS_DAEMON_SPEAK_MAX_CHARS` | `400`                    | Truncation limit (0 = no cut)  |
| `TTS_DAEMON_SPEAK_PROVIDER`  | server default           | TTS provider to use            |
| `TTS_DAEMON_SPEAK_VOICE`     | provider default         | Voice id                       |
| `TTS_DAEMON_SPEAK_SPEED`     | `1.0`                    | Rate multiplier                |

Set them in the hook command itself if you like:

```json
"command": "TTS_DAEMON_SPEAK_SPEED=1.3 python3 ~/.claude/hooks/claude_code_speak.py"
```
